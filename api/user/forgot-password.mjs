...
export default async function handler(req, res) {
  ...
  let demoCode;
  if (user) { const code = Math.floor(100000 + Math.random() * 900000).toString(); user.resetTokenHash = crypto.createHmac("sha256", JWT_SECRET).update(code).digest("hex"); user.resetTokenExpiry = Date.now() + 900000; await saveUsers(users); if (!process.env.RESEND_API_KEY) demoCode = code; }
  return res.status(200).json({ success: true, message: "If an account exists, a 6-digit code has been generated.", demoCode });
}
